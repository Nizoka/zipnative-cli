...
- No `eval()`, `Function()`, or dynamic code execution — with **one declared exception**: `--codec <module>` dynamically imports a user-supplied ESM module and registers its codecs (`registerCodec`, `setInflateImpl`, `setDeflateImpl`). It runs with the invoking user's privileges (the same trust as `node -r`), so it is honoured **only from argv**: `.zipnativerc.json` refuses the `codec` key (a hostile repository cannot run code when you type `zipnative list` inside it), a `batch --manifest` task carrying `codec` is refused unless the invocation itself passes `--allow-codec-load`, and a loaded module is reported truthfully: a codec registered for method 0 or 8 replaces the built-in compressor for `create`/`modify` (even under `--deterministic`, which pins only the engine's own encoder — a `warning:` line says so), a `deflateImpl` shows up as `tier: "injected"` in the envelope, and `create --parallel` refuses to run with either loaded because its workers cannot see the module. `batch --manifest` otherwise dispatches only to a fixed whitelist of CLI command modules.
