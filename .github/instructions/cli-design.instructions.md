...
|------|---------|
| 0 | Success |
| 1 | Runtime error (invalid input, I/O failure, hostile archive, failed check/verify) |
| 2 | Usage error (missing/invalid required argument, `ZIP_INVALID_OPTION`, `ZIP_LIMIT_INVALID`) |
...
- The global-flag block sets `ZIPNATIVE_JSON=1` on `--json`, `ZIPNATIVE_DRY_RUN=1` on
  `--dry-run`, `ZIPNATIVE_QUIET=1` on `--quiet`, `ZIPNATIVE_STRICT=1` on `--strict` and
  `ZIPNATIVE_PURE_CODECS=1` on `--pure-codecs`, so all commands and `utils/agent.ts` can read
  them via env (batch tasks run in-process and inherit them).
...
- One global `USAGE` block listing all 15 commands under their 4 group headings plus the
  `GLOBAL_USAGE` block (the global options incl. the 8 `--max-*` limits), and one `*_USAGE`
  block per command. Point agents at `AGENTS.md`; state "Offline, always".
- Keep `*_USAGE` flag lists in sync with each command's actual flags AND with the `flags`
  array of the same command in `completion.ts`.
