...
| `--format`, `-f` | `text`\|`json`\|`ndjson` | `text` (`ndjson` under `--json`) | Listing format (NDJSON rows are emitted as entries arrive) |
...
**Status envelope:** `{ ok, command: 'crc32', files, bytes }` (stderr, in addition to the stdout report).
...
npm run test:coverage       # thresholds: statements 93 / branches 88 / functions 94 / lines 93
                            # (measured 96.25 / 92.32 / 97.9 / 96.8 on 2026-09-05; never lower them — add tests)
